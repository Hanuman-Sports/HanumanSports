...
function initLoadingLinks() {
    document.querySelectorAll('a').forEach(link => {
        ...
        if (href && href !== '#' && !href.startsWith('http') && !href.startsWith('tel:') && !href.startsWith('mailto:') && !link.classList.contains('no-loader')) {
            link.addEventListener('click', (e) => {
                e.preventDefault();
                localStorage.setItem('loadingTarget', href);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // If we are on the target page, clear the loadingTarget to avoid loops if redirected back
    const currentPath = window.location.pathname.split('/').pop() || 'index.html';
    const target = localStorage.getItem('loadingTarget');
    if (target && (target.toLowerCase() === currentPath.toLowerCase() || target.toLowerCase() === window.location.href.toLowerCase())) {
        localStorage.removeItem('loadingTarget');
    }
});
